...
import * as protos from '@hyperledger/fabric-protos';
...
async function main(): Promise<void> {
    ...
    try {
        ...
        //获取最新区块号
        const blockHeight = await getLatestBlockHeight(network) - 1n;
...
async function getLatestBlockHeight(network: Network): Promise<bigint> {
    const qscc_contract = network.getContract('qscc');
    const resultByte = await qscc_contract.evaluateTransaction('GetChainInfo', channelName);
    const info = protos.common.BlockchainInfo.deserializeBinary(resultByte);
    return BigInt(info.getHeight());
}
