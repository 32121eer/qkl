...
// Path to crypto materials.
const cryptoPath = '/root/czs/fabric/fabric-samples-main/test-network/organizations/peerOrganizations/org1.example.com'
...
async function RegisterChain(contract) {
    console.log(
        '\n--> Submit Transaction: InitLedger, function creates the initial set of assets on the ledger'
    );

    await contract.submitTransaction('RegisterChain');
...
async function main() {
    ...
    try {
        ...
        let chaincodeName = 'test_cc';
        // Get the smart contract from the network.
        const contract = network.getContract(chaincodeName);
        
        await contract.submitTransaction('InitLedger');
        const resultBytes = await contract.submitTransaction('InvokeTest2');
        const resultJson = utf8Decoder.decode(resultBytes);
        const result = JSON.parse(resultJson);
        console.log('*** Result:', result);
